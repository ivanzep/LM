...
| Jams | A full-width grid of mini month calendars at the top — one per month that has any jam, auto-generated from the jam dates (no empty months shown) — with confirmed dates outlined sage and proposed dates outlined red; hovering a marked date shows its time range and location as a native tooltip. Below that, three collapsible sections in order Proposed Dates (expanded by default) / Open Jams (collapsed by default) / Past Jams. Each proposed date is itself a collapsed row (date, time, location, vote-count badges) that expands on click to show per-member availability and the Confirm/Edit/Delete actions, so the section reads as a scannable list rather than a wall of vote rows. A Duration (hr) field in the add/edit form auto-fills End Time from Start Time (supports quarter-hour fractions and wraps past midnight); typing an End Time directly still works and isn't overwritten unless Start Time or Duration changes again |
