...
There's no virtual DOM. `render()` regenerates the entire `#root` innerHTML from `appTemplate()` and is called after any discrete action (add/delete/toggle/nav change/etc). To avoid losing input focus on every keystroke, `render()` snapshots and restores `document.activeElement` (id + selection range) around the DOM replacement.
...
|---|---|
| Songs | Song list grid, filter/search, add modal, and a "Group by" row (None / Artist / Setlist / Rating / Votes) that splits the grid into labeled sections instead of reordering it — a song can appear in more than one section when grouped by Setlist. Shares the playlist player side panel (see below) so you can pick a reference playlist before opening a song |
| Song Detail | Full-page song view with teleprompter auto-scroll (play/pause, speed 1–10, progress %), a 0–5 star rating, and a band voting row (click a member's chip to toggle their vote). The playlist side panel persists across the whole Songs section — switching Lyrics/Tabs/Notes, rating a song, voting, or navigating between songs never interrupts playback, since only the relevant widget re-renders, not the panel |
