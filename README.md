...
| Jams | A row of mini month calendars at the top — one per month that has any jam, auto-generated from the jam dates (no empty months shown) — with confirmed dates outlined sage and proposed dates outlined red; hovering a marked date shows its time range and location as a native tooltip. Below that, three sections: Open / Proposed / Past (all collapsible), availability voting, optional start/end time. A Duration (hr) field in the add/edit form auto-fills End Time from Start Time (supports quarter-hour fractions and wraps past midnight); typing an End Time directly still works and isn't overwritten unless Start Time or Duration changes again |
