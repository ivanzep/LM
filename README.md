...
| Song Detail | Full-page song view with teleprompter auto-scroll (play/pause, speed 1–10, progress %), a 0–5 star rating, and a band voting row (click a member's chip to toggle their vote). The playlist side panel persists across the *entire* Songs section — filtering, grouping, searching, opening/closing a song, switching Lyrics/Tabs/Notes, rating, or voting never interrupt playback, since `render()` detaches and reattaches the live iframe node around every re-render rather than letting it get recreated |
